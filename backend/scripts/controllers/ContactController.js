angular.module('e-homework').controller('ContactController', function($scope, $compile, $cookies, $filter, $state, HTTPService, IndexOverlayFactory) {
    ...
    $scope.save = function(action, ContactObj, AttachFileObj){
        // console.log($scope.Contact);
        IndexOverlayFactory.overlayShow();
        ContactObj.contents = CKEDITOR.instances.editor1.getData();
        ContactObj.contents_en = CKEDITOR.instances.editor2.getData();

        var params = {'ContactObj':ContactObj, 'AttachFileObj':AttachFileObj};
